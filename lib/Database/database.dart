...
class Database {
  ...
  void updateCart(int appid, int quantity) async {
    ...
    String query;
    if (quantity <= 0) {
      query = "delete from user_cart where appid = $appid";
    } else {
      query = "update user_cart set quantity = $quantity where appid = $appid";
    }
  ...
  Future<List<Order>> getOrders() async {
    ...
    for (final row in results) {
      query =
          "select * from orders natural join game_order natural join game natural join publisher natural join developer  where order_id =${row[3]}";
...
//Order(this.orders, this.date, this.card, this.streetNo, this.street, this.city, this.country, this.orderId);
      orders.add(Order(games, date, row[4], row[0], row[1], row[2], row[8],
          row[9], row[3], row[6]));
    }
    await connection.close();
    return orders;
  ...
  Future<List<Game>> getRecommended() async {
    await connection.open();
    String query =
        "with x as ( WITH agg AS ( select genre, count(*) from game_gen natural join game natural join game_order natural join orders where email = '$email'"
  ...
  void removeGame(int appid) async {
    await connection.open();
    String query = "update game set available = false where appid = ${appid}";
    await connection.query(query);
  }
  ...
  Future<bool> checkOut(
      Map<String, String> address, Map<String, String> bank, context) async {
    await connection.open();
    ...
    query =
        "insert into address values ('${address['Street Number']}', '${address['Street']}', '${address['City']}', '${address['Province']}', '${address['Country']}') on conflict do nothing";
    print(query);
    await connection.query(query);
    //bank_info(card, email, street_no, street, city, first_name, last_name)
    query =
        "insert into bank_info values ('${bank['Card']}', '$email', '${address['Street Number']}', '${address['Street']}', '${address['City']}', '${bank['First']}', '${bank['Last']}') on conflict do nothing";
    print(query);
    await connection.query(query);
    //user_bank(email, card)
    query =
        "insert into user_bank values ('$email', '${bank['Card']}') on conflict do nothing";
    print(query);
    await connection.query(query);
    ...
    query =
        "insert into orders values ($orderId, '${bank['Card']}', '$email', '${address['Street Number']}', '${address['Street']}', '${address['City']}', $orderId,  ${DateTime.now().millisecondsSinceEpoch})";
    print(query);
    await connection.query(query);
    for (Game g in cart) {
      query =
          "insert into game_order values (${g.appid}, $orderId, ${g.quantity})";
      print(query);
      await connection.query(query);
      int x = ((g.quantity * g.price) * (g.sellprice / 100)).toInt();
      query =
          "select pub_email from publisher where pub_name = '${g.publisher}' ";
      String pub = "none";
      var results = await connection.query(query);
      for (final row in results) pub = row[0];
      query =
          "INSERT INTO amount_owed VALUES ('$pub', '$email', $x) ON CONFLICT (pub_email) DO UPDATE SET amount = excluded.amount + $x";
      print(query);
      await connection.query(query);
      query =
          "update game_ware set quantity = quantity-${g.quantity} where appid = ${g.appid}";
      print(query);
      await connection.query(query);
    }
  ...
  Future<Map<String, List<int>>> getPublisherData() async {
    ...
    Map<String, List<int>> genreData = new Map<String, List<int>>();
    String query =
        "select sum(quantity)::smallint, pub_name, sum(sell_price), avg(percentage)::smallint from game_order natural join game natural join publisher group by pub_name";
    var results = await connection.query(query);
    for (final row in results) {
      List<int> i = new List<int>();
      i.add(row[0]);
  ...
  Future<Map<String, List<int>>> getDeveloperData() async {
    ...
    Map<String, List<int>> genreData = new Map<String, List<int>>();
    String query =
        "select sum(quantity)::smallint, dev_name, sum(sell_price), avg(percentage)::smallint from game_order natural join game natural join developer group by dev_name";
    var results = await connection.query(query);
    for (final row in results) {
      List<int> i = new List<int>();
      i.add(row[0]);
...
//      print(query);
  ...
  ownerAdd(
    ...
    query =
        ...
        ",$ratings,$sellPrice, $percentage, true) on conflict do nothing";
//      print(query);
    await connection.query(query);
